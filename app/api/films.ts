...
export async function getFilms(title?: string | null) {
  const respon = await fetch("https://ghibliapi.herokuapp.com/films");
  const films: Film[] = await respon.json();
...
export async function getFilmById(filmId: string) {
  const respon = await fetch(`https://ghibliapi.herokuapp.com/films/${filmId}`);
  const film: Film = await respon.json();
